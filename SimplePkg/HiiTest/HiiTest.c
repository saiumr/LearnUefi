  ...
  )
{
  ...
  StrBuf = HiiGetStringEx(HiiHandle, STRING_TOKEN(STR_LANGUAGE_SELECT), "en-US", TRUE);                      // use lib, and another way is using protocol
  Print(L"%s\n", StrBuf);
  gBS->FreePool(StrBuf);
  StrBuf = NULL;
  ...
  StrBuf = HiiGetStringEx(HiiHandle, STRING_TOKEN(STR_LANGUAGE_SELECT), "zh-Hans", TRUE);   // system does not support zh-Hans, so here is blank.
  Print(L"%s\n", StrBuf);
  gBS->FreePool(StrBuf);
  StrBuf = NULL;
