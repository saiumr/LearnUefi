/** The program use this library could parse (*gif) picture into a specified data structure(GIF), 
 *  which contain all of raw data (File header and all of image frame data) of a gif picture.  
 *  Now you can use internal function convert every gif frame to bmp (24bit) picture.  
 *  Sure, the specified data structure should be useful, more operations on gif images that are not available here can be completed with it.  
 *  The GIFParserLogoBltFromFileOrBuffer is not an library function strictly, but an application, which parse gif picture and blt every frame to display.  
 **/
#ifndef _GIF_PARSER_
#define _GIF_PARSER_
