...
static UINTN gen_mask(UINTN len) {
  UINTN mask;
...
struct darray *lzw_bw_result(struct lzw_bit_writer *b) {
  if (!b) return NULL;
  if (b->pos) {
    dapush(b->buf, (UINT8) (b->bits & gen_mask(b->pos)));
  }
  return b->buf;
}
...
INT32 lzw_br_read(struct lzw_bit_reader *r,
  ...
  while (r->pos < n_bits) {
    ...
    tmp = (UINTN)(r->buf.data[r->buf.pos++]);
    r->bits |= (tmp & gen_mask(8)) << r->pos;
    r->pos += 8;
  }
  *result = r->bits & (UINTN)gen_mask(n_bits);
  r->pos -= n_bits;
