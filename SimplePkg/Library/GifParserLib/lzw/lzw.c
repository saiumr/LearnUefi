/* This file is a part of liblzw
 *
 * Copyright 2019, Jeffery Stager
 *
 * liblzw is FreePool software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * liblzw is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with liblzw.  If not, see <https://www.gnu.org/licenses/>.
 */
#include "lzw.h"
#include "lzw_bits.h"
#include "lzw_table.h"

VOID compress(UINT8 bit_size,
              UINTN size,
              UINT8 *src,
              UINTN *out_len,
              UINT8 **output,
              INT32 gif_format) {
  UINT8 bit_width = bit_size + 1;
  struct lzw_bit_writer b;
  struct lzw_table ctable;
  struct darray *result;

  if (!output) return;
  if (!out_len) return;
  lzw_bw_init(&b, BIT_BUFFER, NULL);
  lzw_table_init(&ctable, LZW_TABLE_COMPRESS, bit_size);
  if (gif_format != 0) lzw_bw_pack(&b, bit_width, (UINTN)(1 << bit_size));
  for (;;) {
    UINTN code = 0;
    struct lzw_entry e = { 0 };

    /* build up the next dictionary entry */
    do {
      e.val = *src++;
      e.prev = code;
      e.len++;
      size--;
      if (size == 0) {
        lzw_bw_pack(&b, bit_width, code);
        code = *--src;
        break;
      }
    } while (lzw_table_lookup_entry(&ctable, &e, &code));
    lzw_table_add(&ctable, &e);
    lzw_bw_pack(&b, bit_width, code);
    if (ctable.n_entries > (UINTN)(1 << bit_width)) bit_width++;
    if (size == 0) break;
    size++;
    src--;

  }
  /* end of stream code for GIFs */
  if (gif_format != 0) lzw_bw_pack(&b, bit_width, (UINTN)((1 << bit_size) + 1));
  lzw_table_deinit(&ctable);
  result = lzw_bw_result(&b);
  *out_len = (UINTN)dalen(result);
  *output = dapeel(result);
}

/* **************************************** */
/* Public */
/* **************************************** */

VOID lzw_compress(UINT8 bit_size,
                  UINTN size,
                  UINT8 *src,
                  UINTN *out_len,
                  UINT8 **result) {
}

VOID lzw_compress_gif(UINT8 bit_size,
                      UINTN size,
                      UINT8 *src,
                      UINTN *out_len,
                      UINT8 **result) {
  compress(bit_size, size, src, out_len, result, 1);
}

VOID lzw_decompress(UINT8 bit_size,
                    UINTN size,
                    UINT8 *src,
                    UINTN *out_len,
                    UINT8 **result) {
  UINT8 bit_width = bit_size + 1;
  UINTN code;
  struct lzw_bit_reader b;
  struct lzw_table dtable;
  struct darray *output;

  if (!src) return;
  if (!result) return;
  if (!out_len) return;
  output = danew(4096);
  lzw_table_init(&dtable, LZW_TABLE_DECOMPRESS, bit_size);
  lzw_br_init(&b, BIT_BUFFER, size, src);

  while (lzw_br_read(&b, bit_width, &code)) {
    struct lzw_entry cur;

    /* LZW clear code */
    if (code == (UINTN)(1 << bit_size)) {
      bit_width = (UINT8)(bit_size + 1);
      dtable.n_entries = (UINTN)((1 << bit_size) + 2);
      continue;
    }
    /* explicit error code */
    if (code == (UINTN)((1 << bit_size) + 1)) break;
    if (lzw_table_lookup_code(&dtable, code, &cur)) {
      UINTN next_code;
      UINTN i;
      struct darray *buf;
      struct lzw_entry new, next;

      lzw_table_str(&dtable, code, &buf);
      for (i = dalen(buf); i > 0; --i) {
        dapush(output, daptr(buf)[i-1]);
      }
      dafree(buf);
      if ((dtable.n_entries + 1 > (UINTN)(1 << bit_width)) &&
          (bit_width < LZW_MAX_ENTRY_EXP)) {
        bit_width++;
      }
      if (lzw_br_peek(&b, bit_width, &next_code) == 0) continue;
      new.len = cur.len + 1;
      if (lzw_table_lookup_code(&dtable, next_code, &next) != 0) {
        new.val = lzw_entry_head(&dtable, &next);
      } else {
        new.val = lzw_entry_head(&dtable, &cur);
      }
      new.prev = code;
      lzw_table_add(&dtable, &new);
    }
  }
  lzw_table_deinit(&dtable);
  *out_len = dalen(output);
  *result = dapeel(output);
}
