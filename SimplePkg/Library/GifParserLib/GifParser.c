...
VOID _PrintBuffer(IN CHAR8 *buffer, IN UINTN len);
...
GIFParserGetAnimationFromGif(IN GIF *gif, OUT IMG_ANIMATION **animation)
{
    ...
    *animation = (IMG_ANIMATION *)AllocatePool(sizeof(IMG_ANIMATION));
    ...
    while (image != NULL)
    {
        ...
        if (changed_data)
            FreePool(changed_data);
        changed_data = NULL;
        if (changed_color_index_list)
            FreePool(changed_color_index_list);
        changed_color_index_list = NULL;
    }

    if (color_index_list)
        FreePool(color_index_list);
    if (color_index_kind)
        FreePool(color_index_kind);
    return TRUE;
}
...
_HandleImageData(IN CHAR8 **src, OUT GIF **gif)
{
    ...
    *src -= 1; // already read ','  -> 0x2C
    ...
    if ((new_image_node->image_descriptor.flag_color_table & 0x1) == 1)
    { // local color table
